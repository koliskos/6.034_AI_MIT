...
def extensions(graph, path):
    ...
    #get last node in given path
    lastN = path[-1]
    #then get the neibhors of the last node in a list named near
    near = graph.get_neighbors(lastN)
    #then loop through near check if neibhor is not in path,
    # add neibhor edge t0 list of extensions
...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    node name."""
    lenNs = []
    #for loop through each node in graph to goal node
    for n in nodes:
        toG = graph.get_heuristic_value(n, goalNode)
        lenNs.append((n,toG))
    byLen = sorted(lenNs, key=lambda entry: entry[0])
    byLex = sorted(byLen, key=lambda entry: entry[1])
    toReturn = [tup[0] for tup in byLex]
    return toReturn
...
def basic_dfs(graph, startNode, goalNode):
    ...
    """
    #Remove first path from agenda. If it ends with the goal node, return that path.
    #agenda = startNode
    #if agenda = goalNode:
    #    return agenda
    #Otherwise, extend the path, and add all children to the agenda.
 #Repeat until there is nothing left in the agenda to be removed, in which case terminate with failure.
    #if start doesnt have neibhors, then return

    pathStack = [[startNode]]


    while len(pathStack) > 0:
        if pathStack[0][-1] == goalNode:
            return pathStack[0]
        else:
            toExamine = pathStack.pop(0)
            addToAgenda = extensions(graph, toExamine)
            pathStack = addToAgenda + pathStack
...
def basic_bfs(graph, startNode, goalNode):
    ...
    while len(pathQueue) > 0:
        if pathQueue[0][-1] == goalNode:
            return pathQueue[0]
        else:
            toExamine = pathQueue.pop(0)
            addToAgenda = extensions(graph, toExamine)
            pathQueue =  pathQueue + addToAgenda
...
# def my_sort_new_paths_fn(graph, goalNode, paths):
#
#     return sorted_paths
...
def my_hill_climb_sort(graph,goalNode,paths):
    paths = sorted(paths)
    paths = sorted(paths, key=lambda path: graph.get_heuristic_value(path[len(path)-1], goalNode))
    return paths
generic_hill_climbing = [my_hill_climb_sort, True, do_nothing_fn, False]
def my_generic_best_first(graph, goalNode, paths):
    paths = sorted(paths, key = lambda path: graph.get_heuristic_value(path[len(path)-1], goalNode))
    return sorted_paths
generic_best_first = [do_nothing_fn, True, my_generic_best_first, False]
def my_branch_sort(graph,goalNode,paths):
    queue = sorted(paths, key=lambda path: path_length(graph, path))
    return queue
generic_branch_and_bound = [do_nothing_fn, False, my_branch_sort, False]
def my_heuristicpath_sort(graph, goalNode, paths):
    agenda = sorted(paths, key= lambda path: path_length(graph,path) +graph.get_heuristic_value(path[-1], goalNode))
    return agenda
generic_branch_and_bound_with_heuristic = [do_nothing_fn, False, my_heuristicpath_sort, False]
generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, my_branch_sort, True]
generic_a_star = [do_nothing_fn, False, my_heuristicpath_sort, True]
...
def is_admissible(graph, goalNode):
    ...
    for node in graph.nodes:
        if path_length(graph, my_sort_branch_and_bound(graph,node,goalNode)) < graph.get_heuristic_value(node,goalNode):
            return False
    return True
...
WHAT_I_FOUND_INTERESTING = "DFS and BFS"
WHAT_I_FOUND_BORING = "Heuristics vs cost"
SUGGESTIONS = None
